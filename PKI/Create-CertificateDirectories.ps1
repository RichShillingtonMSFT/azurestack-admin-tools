...
$AppServiceSubDirectories | % { New-Item -Path (Join-Path $AppServicesDirectory $PSITEM) -ItemType Directory -Force}

# Other Folders
$OtherFoldersDirectory = 'C:\Certificates\'
$OtherFolders = 'DBAdapter','EventHubs','IoTHub'
$OtherFolders | % { New-Item -Path (Join-Path $OtherFoldersDirectory $PSITEM) -ItemType Directory -Force}
