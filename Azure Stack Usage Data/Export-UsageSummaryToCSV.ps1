<#
.SYNOPSIS
    Script to export Azure Stack Usage Summary to CSV

.DESCRIPTION
    This script is used to to export Azure Stack Usage Summary to CSV.
    The script will prompt you to login in with your Azure Stack Operator Credentials.
    You must then select the Default Provider Subscription

.PARAMETER StartTime
    Provide an start time.
    Default is current is 24 hours ago
...
.PARAMETER Granularity
    Provide the granularity for the report.
    Example: "Hourly", "Daily"
    Default is Daily

.PARAMETER CsvFile
    Provide the full path to export the CSV file.
    Default is [UserProfile]\Documents\UsageSummary-[date].csv

.EXAMPLE
    .\Export-UsageSummaryToCSV.ps1
#>
[CmdletBinding()]
Param
(   
...
if (!$StartTime)
{
    ...
    $StartTime = (Get-Date $StartTime -Format "yyyy-MM-ddT00:00") + "+00:00Z"
}
if (!$EndTime)
{
    $EndTime = (Get-Date -Format "yyyy-MM-ddT00:00") + "+00:00Z"
}
if (!$CsvFile)
{
    $CSVFileName = 'UsageSummary-' + $(Get-Date -f yyyy-MM-dd) + '.csv'
    $CsvFile = "$env:USERPROFILE\documents\$CSVFileName"
}
if (!$Granularity)
{
    $Granularity = 'Daily'
...
#region Build a subscription hashtable
$subtable = @{}
