Function Get-HyperVVMMigrationReadinessReport 
{
    
    [CmdletBinding()]
    Param 
    (
        # Specify the Name or IP of the Hypervisor Host
	    [Parameter(Mandatory=$true,HelpMessage="Specify the Name or IP of the Hypervisor Host. Example Host-001")]
	    [String][String]$HypervisorHost,

        # Specify the credentials of the Hypervisor Host
        [Parameter(Mandatory=$false,HelpMessage="Specify the credentials of the Hypervisor Host. Get-Credential")]
    ...
    Write-Host "Getting list of VMs from $HypervisorHost. Please wait..." -ForegroundColor Green
    $HypervisorHostVMs = Get-VM -ComputerName $HypervisorHost -Credential $Credentials | Select-Object -Property *

    $RequirementChecks = @(
        @{RequiredGeneration = "1"; IfFalseMessage = "The VM Must be Generation 1. You will need to recreate your VM in your hypervisor."}
        @{RequiredVhdType = "Fixed"; IfFalseMessage = "The VM Must Use Fixed Disks. You can convert the Dynamic Disk to Fixed using Convert-VHD"}
        @{RequiredVhdFormat = "VHD"; IfFalseMessage = "Each VM Disk Must Be VHD Format. You can convert the VHDX to VHD using Convert-VHD"}
    )
    ...
    foreach ($VM in $HypervisorHostVMs)
    {
        $VMCheckFailCount = 0

        Write-Host "Checking VM Generation" -ForegroundColor Green
        if ($VM.Generation -ne $RequirementChecks.RequiredGeneration)
        {
            Write-Warning -Message "$($VM.Name) on Host $HypervisorHost is NOT ready to Migrate. $($RequirementChecks[0].IfFalseMessage)"
            $MigrationNotReadyVMs += New-Object PSObject -Property ([ordered]@{VMName=$($VM.Name);VMHost=$HypervisorHost;Message=$($RequirementChecks[0].IfFalseMessage)})
            $VMCheckFailCount++
        }

        Write-Host "Getting VM Disk Information" -ForegroundColor Green
        $VMDisks = $VM.VMId | Get-VHD -Credential $Credentials -ComputerName $HypervisorHost | Select-Object -Property *
        ...
        foreach ($VMDisk in $VMDisks)
        {
            ...
            if ($VMDisk.VhdFormat -ne $RequirementChecks.RequiredVhdFormat)
            {
                Write-Warning -Message "$($VM.Name) on Host $HypervisorHost is NOT ready to Migrate. Disk $DiskName format is $($VMDisk.VhdFormat). $($RequirementChecks[2].IfFalseMessage)"
                $MigrationNotReadyVMs += New-Object PSObject -Property ([ordered]@{VMName=$($VM.Name);VMHost=$HypervisorHost;Disk=$DiskName;DiskType=$($VMDisk.VhdType);Message=$($RequirementChecks[1].IfFalseMessage)})
                $VMCheckFailCount++
            }

            Write-Host "Checking VM Disk Type" -ForegroundColor Green
            if ($VMDisk.VhdType -ne $RequirementChecks.RequiredVhdType)
            {
                Write-Warning -Message "$($VM.Name) on Host $HypervisorHost is NOT ready to Migrate. Disk $DiskName type is $($VMDisk.VhdType). $($RequirementChecks[1].IfFalseMessage)"
                $MigrationNotReadyVMs += New-Object PSObject -Property ([ordered]@{VMName=$($VM.Name);VMHost=$HypervisorHost;Disk=$DiskName;DiskType=$($VMDisk.VhdType);Message=$($RequirementChecks[1].IfFalseMessage)})
                $VMCheckFailCount++
            }
            ...
            if ($CurrentDiskSizeInGB -notin $($RequiredDiskSizes.SizeInGB))
            {
                ...
                $Message = "$($VM.Name) Disk $DiskName is currently $CurrentDiskSizeInGB GB. You can resize the disk to $RecommendedDiskSize GB using Resize-VHD"
                Write-Warning -Message $Message
                $MigrationNotReadyVMs += New-Object PSObject -Property ([ordered]@{VMName=$($VM.Name);VMHost=$HypervisorHost;Disk=$DiskName;DiskSize=$CurrentDiskSizeInGB;Message=$Message})
                $VMCheckFailCount++
            }
        }
        ...
        {
            Write-Host "Virtual Machine $($VM.VMName) on host $HypervisorHost is ready to migrate."
            $MigrationReadyVMs += New-Object PSObject -Property ([ordered]@{VMName=$($VM.Name);VMHost=$HypervisorHost;Status='Ready To Migrate'})
        }
    }

    $MigrationReadyVMs | Export-Csv $FileSaveLocation\$('VMsReadyForMigration-' + $(Get-Date -f yyyy-MM-dd) + '.csv') -NoTypeInformation
    $MigrationNotReadyVMs | Export-Csv $FileSaveLocation\$('VMsNOTReadyForMigration-' + $(Get-Date -f yyyy-MM-dd) + '.csv') -NoTypeInformation
}
Export-ModuleMember -Function Get-HyperVVMMigrationReadinessReport
...
Function Invoke-HyperVCreateDiskBackup
{
    [CmdletBinding()]
    Param 
    (
        ...
        # Specify the credentials of the Hypervisor Host
        [Parameter(Mandatory=$true,HelpMessage="Specify the credentials of the Hypervisor Host. Get-Credential")]
        [pscredential]$Credentials
    )

    try 
    {
        $VirtualMachine = Get-VM -Name $VirtualMachineName -ComputerName $HypervisorHost -Credential $Credentials -Verbose -ErrorAction Stop | Select-Object -Property *
    }
    catch 
    {
        Write-Warning "Unable to get information about $VirtualMachineName from host $HypervisorHost"
        Write-Warning "Please verify your connection and credentials and try again."
        break
    }
    ...
    {
        Write-Host "Getting VM Disk Information" -ForegroundColor Green
        $VMDisks = $VirtualMachine.VMId | Get-VHD -Credential $Credentials -ComputerName $HypervisorHost -Verbose -ErrorAction Stop | Select-Object -Property *
    }
    catch 
    {
        Write-Warning "Unable to get VM Disk information about $VirtualMachineName from host $HypervisorHost"
        Write-Warning "Please verify your connection and credentials and try again."
        break
    }
    ...
    if ($VirtualMachine.State -ne 'Off')
    {
        ...
        Stop-VM -Name $VirtualMachineName -ComputerName $HypervisorHost -Credential $Credentials -Verbose 
        while ((Get-VM -Name $VirtualMachineName -ComputerName $HypervisorHost -Credential $Credentials).State -ne 'off') 
        {
    ...
    $Session = New-PSSession -ComputerName $HypervisorHost -Credential $Credentials

    foreach ($VMDisk in $VMDisks)
    {
        [String]$CurrentDiskExtention = ('.' + $VMDisk.VhdFormat).ToLower()
        [String]$BackupDiskExtension = ('-BkUp' + $CurrentDiskExtention).ToLower()
        [String]$BackupDiskPath = $VMDisk.Path.ToString().Replace("$CurrentDiskExtention", "$BackupDiskExtension")
        Write-Host "Backing up $($VMDisk.Path) to $BackupDiskPath. This may take a while.."
        Invoke-Command -Session $Session -ScriptBlock {
            try 
            {
                Copy-Item $Using:VMDisk.Path $Using:BackupDiskPath -ErrorAction Stop -Verbose 
                Write-Host "Disk Backup to $BackupDiskPath is complete" -ForegroundColor Green
            }
            catch 
            {
                Write-Warning $_
    ...
    Write-Host "Disk Backup Complete" -ForegroundColor Green
}
Export-ModuleMember -Function Invoke-HyperVCreateDiskBackup

Function Invoke-HyperVVHDXToVHDAndDynamicToFixedConversion 
{
    [CmdletBinding()]
    Param 
    (
    ...
    $VirtualMachine = Get-VM -Name $VirtualMachineName -ComputerName $HypervisorHost -Credential $Credentials -Verbose | Select-Object -Property *

    Write-Host "Getting VM Disk Information" -ForegroundColor Green
    $VMDisks = $VirtualMachine.VMId | Get-VHD -Credential $Credentials -ComputerName $HypervisorHost -Verbose | Select-Object -Property *
    ...
    foreach ($VMDisk in $VMDisks)
    {
        ...
        {
            $DiskName = $($VMDisk.Path.Split('\') | Select-Object -Last 1)
            $NewDiskPath = $VMDisk.Path.ToString().ToLower().Replace('vhdx','vhd')

            Write-Host "Checking VM Disk Type" -ForegroundColor Green
            if ($VMDisk.VhdType -ne $RequirementChecks.RequiredVhdType)
            {
                Write-Warning -Message "$($VirtualMachine.Name) on Host $HypervisorHost disk $DiskName type is $($VMDisk.VhdType)"
                Write-Warning -Message "Disk $DiskName will be converted to Fixed using Convert-VHD. This may take a while..."
                Invoke-Command -Session $Session -ScriptBlock {
                    try 
                    {
                        Convert-VHD -Path $Using:VMDisk.Path -DestinationPath $Using:NewDiskPath -VHDType Fixed -ErrorAction Stop -Verbose
                    }
                    catch
                    {
                        Write-Warning $_
                ...
                Write-Host "Disk $DiskName is Now a Fixed Size VHD" -ForegroundColor Green
            }
            else 
            {
                Write-Warning -Message "$($VirtualMachine.Name) on Host $HypervisorHost Disk $DiskName format is $($VMDisk.VhdFormat)."
                Write-Warning -Message "Disk $DiskName will be converted to VHD. This may take a while..."
                Invoke-Command -Session $Session -ScriptBlock {
                    try 
                    {
                        Convert-VHD -Path $Using:VMDisk.Path -DestinationPath $Using:NewDiskPath -VHDType Fixed -ErrorAction Stop -Verbose
                ...
                Write-Host "Disk $DiskName is Now a VHD" -ForegroundColor Green
            }
        }
    }

    Remove-PSSession $Session
    Write-Host "All Disks are now VHD Format and Fixed Size" -ForegroundColor Green
}
Export-ModuleMember -Function Invoke-HyperVVHDXToVHDAndDynamicToFixedConversion

Function Invoke-HyperVVHDRightSizing 
{
    [CmdletBinding()]
    Param 
    (
    ...
    {  
        ...
        if ($VMDisk.Path -like "*.vhdx")
        {
            $NewDiskPath = $VMDisk.Path.ToString().ToLower().Replace('vhdx','vhd')
        }
        else 
        {
            $NewDiskPath = $VMDisk.Path.ToString().ToLower()
        }
        ...
        if ($CurrentDiskSizeInGB -notin $($RequiredDiskSizes.SizeInGB))
        {
            ...
            Write-Warning -Message $Message
            Write-Host "Resizing Disk $NewDiskPath. This may take a while..."
            Invoke-Command -Session $Session -ScriptBlock {
                try
                {
                    Resize-VHD -Path $Using:NewDiskPath -SizeBytes $Using:NewDiskSizeInBytes -Verbose
                }
                catch
                {
                    Write-Warning $_
            ...
            Write-Host "Disk $NewDiskPath is now $($RecommendedDiskSize + 'GB')" -ForegroundColor Green
        }
    }
...
Function Get-HyperVAzureVirtualMachineSizeFromHost
{
    [CmdletBinding()]
    Param 
    (
    ...
    Write-Host "Getting VM Information from $HypervisorHost. Please wait..." -ForegroundColor Green
    $VirtualMachine = Get-VM -Name $VirtualMachineName -ComputerName $HypervisorHost -Credential $Credentials | Select-Object -Property *
    ...
    $AzureStackVMSizeList | Where-Object {($_.NumberOfCores -ge $VirtualMachine.ProcessorCount) -and ($_.MemoryInMB -ge ($VirtualMachine.MemoryAssigned/1mb)) -and ($_.MaxDataDiskCount -ge $VirtualMachine.HardDrives.Count)} | Sort-Object Name,NumberofCores,MemoryInMB
...
function Invoke-HyperVValidateMigrationReady
{
    [CmdletBinding()]
    param 
    (
    ...
    Write-Host "Validating that the VM is still ready to migrate." -ForegroundColor Green
    Write-Host "Getting Virtual Machine Details from $HypervisorHost. Please wait..." -ForegroundColor Green
    ...
    Write-Host "Checking VM Generation" -ForegroundColor Green
    if ($VirtualMachine.Generation -ne $RequirementChecks.RequiredGeneration)
    {
        Write-Warning -Message "$($VirtualMachine.Name) on Host $HypervisorHost is NOT ready to Migrate. $($RequirementChecks[0].IfFalseMessage)"
        $VMCheckFailCount++
    }

    Write-Host "Getting VM Disk Information" -ForegroundColor Green
    $VMDisks = $VirtualMachine.VMId | Get-VHD -Credential $Credentials -ComputerName $HypervisorHost | Select-Object -Property *
    ...
    foreach ($VMDisk in $VMDisks)
    {
        ...
        if ($VMDisk.VhdFormat -ne $RequirementChecks.RequiredVhdFormat)
        {
            Write-Warning -Message "$($VirtualMachine.Name) on Host $HypervisorHost is NOT ready to Migrate. Disk $DiskName format is $($VMDisk.VhdFormat). $($RequirementChecks[2].IfFalseMessage)"
            $VMCheckFailCount++
        }
        ...
        if ($VMDisk.VhdType -ne $RequirementChecks.RequiredVhdType)
        {
            Write-Warning -Message "$($VirtualMachine.Name) on Host $HypervisorHost is NOT ready to Migrate. Disk $DiskName type is $($VMDisk.VhdType). $($RequirementChecks[1].IfFalseMessage)"
            $VMCheckFailCount++
        }
        ...
        {
            $RecommendedDiskSize = ($RequiredDiskSizes.SizeInGB | Where-Object {$_ -ge $CurrentDiskSizeInGB})[0]
            $Message = "$($VirtualMachine.Name) Disk $DiskName is currently $CurrentDiskSizeInGB GB. You can resize the disk to $RecommendedDiskSize GB using Resize-VHD"
            Write-Warning -Message $Message
            $VMCheckFailCount++
        }
    }
    ...
    if ($VMCheckFailCount -eq 0)
    {
        Write-Host "Virtual Machine $($VirtualMachine.VMName) on host $HypervisorHost is ready to migrate."
    }
    
    return $VMCheckFailCount
}
Export-ModuleMember -Function Invoke-HyperVValidateMigrationReady

Function Invoke-RepairedVMImageCopyHyperVToAzureStackHub
{
    [CmdletBinding()]
    Param
    (
    ...
    Write-Host "Getting VM Information" -ForegroundColor Green
    $VirtualMachine = Get-VM -Name $VirtualMachineName -ComputerName $HypervisorHost -Credential $Credentials -Verbose | Select-Object -Property *
    ...
    foreach ($VMDisk in $VMDisks)
    {
        if ($VMDisk.Path -like "*.vhdx")
        {
            $DiskName = $($VMDisk.Path.Split('\') | Select-Object -Last 1).ToString().ToLower().Replace('vhdx','vhd')
            $NewDiskPath = $VMDisk.Path.ToString().ToLower().Replace('vhdx','vhd')
        }
        else 
        {
            $DiskName = $($VMDisk.Path.Split('\') | Select-Object -Last 1).ToString().ToLower()
            $NewDiskPath = $VMDisk.Path.ToString().ToLower()
        }

        $VirtualHardDrivePath = ('\\' + $HypervisorHost + '\' + $NewDiskPath.Replace(':','$'))
        $VirtualHardDriveRootPath =  $VirtualHardDrivePath.SubString(0, $VirtualHardDrivePath.LastIndexOf('\'))

        # Create PS Drive to Remote Share
        $PSDriveLetter = (Get-ChildItem Function:[f-z]: -n | Where-Object { !(test-path $_) } | Select-Object -First 1)
        $PSDrive = New-PSDrive -Name ($PSDriveLetter.Replace(':','')) -Credential $Credentials -PSProvider FileSystem -Root $VirtualHardDriveRootPath -Persist
        ...
        Invoke-VHDUploadToStorageAccount -StorageAccountDestinationURL $StorageAccountDestinationURL -VHDFilePath "$PSDriveLetter\$DiskName"
        $PSDrive | Remove-PSDrive -Force

        $ControllerNumber = ($VirtualMachine.HardDrives | Where-Object {$_.path -like "*\$($DiskName.replace('vhd','vhdx'))"}).ControllerNumber
        $ControllerLocation = ($VirtualMachine.HardDrives | Where-Object {$_.path -like "*\$($DiskName.replace('vhd','vhdx'))"}).ControllerLocation
        ...
        $DiskData += New-Object PSObject -Property ([ordered]@{VMName=$($VirtualMachine.Name);DiskName=$DiskName;DiskURL=$DiskURL;ControllerNumber=$ControllerNumber;ControllerLocation=$ControllerLocation})
    }

    # Remove Persistent Drive
    net use * /d /y | Out-Null

    $DiskData | Sort-Object -Property ControllerNumber,ControllerLocation | Export-Csv $FileSaveLocation\$($($VirtualMachine.Name) + '-DiskData' + '.csv') -NoTypeInformation
}
Export-ModuleMember -Function Invoke-RepairedVMImageCopyHyperVToAzureStackHub

Function Invoke-HyperVVMImageCopyToAzureStackHub
{
    [CmdletBinding()]
    Param 
    (
    ...
    #region Re-Check VM Migration Readiness
    $ReadinessTest = Invoke-HyperVValidateMigrationReady -VirtualMachineName $VirtualMachineName -HypervisorHost $HypervisorHost -Credentials $Credentials
    if ($ReadinessTest -ne 0)
    {
        Write-Warning -Message "The Virtual Machine $VirtualMachineName had $ReadinessTest Errors."
        Write-Warning -Message "Please Rerun Get-HyperVVMMigrationReadinessReport and fix any issues"
        break
    }
    #endregion
    ...
    {
        $DiskName = $($VMDisk.Path.Split('\') | Select-Object -Last 1).ToString().ToLower()
        $DiskPath = $VMDisk.Path.ToString().ToLower()

        $VirtualHardDrivePath = ('\\' + $HypervisorHost + '\' + $DiskPath.Replace(':','$'))
        $VirtualHardDriveRootPath =  $VirtualHardDrivePath.SubString(0, $VirtualHardDrivePath.LastIndexOf('\'))
        ...
        $ControllerNumber = ($VirtualMachine.HardDrives | Where-Object {$_.path -like "*\$DiskName"}).ControllerNumber
        $ControllerLocation = ($VirtualMachine.HardDrives | Where-Object {$_.path -like "*\$DiskName"}).ControllerLocation
        $DiskURL = $DestinationStorageAccount.Context.BlobEndPoint + $DestinationStorageAccountContainerName.Trim() + '/' + $VirtualMachineName + '/'+ $DiskName
...
Export-ModuleMember -Function Invoke-HyperVVMImageCopyToAzureStackHub
...
Function New-AzureStackVirtualMachineFromHyperVAndDataFile
{
    ...
    $AzureStackVMSizeList = Get-HyperVAzureVirtualMachineSizeFromHost -VirtualMachineName $VirtualMachineName -HypervisorHost $HypervisorHost -Credentials $Credentials
